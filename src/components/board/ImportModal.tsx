...
export const ImportModal = ({ isOpen, onClose }: ImportModalProps) => {
  const { importLeadsWithMapping, getCSVHeaders, getJSONFields } = useBoard();
  ...
  const handleFileImport = (content: string, fileName: string) => {
    ...
    try {
      ...
      setErrorMessage(
        "Failed to analyze file. Please check the file format and try again."
      );
  ...
  const handleMappingComplete = (mappings: FieldMapping[]) => {
    ...
    try {
      importLeadsWithMapping(
        fileContent,
        mappings,
        importType as "csv" | "json"
      );
      setSuccessMessage(
        `${importType.toUpperCase()} file imported successfully with custom field mapping!`
      );
      setShowMappingModal(false);
    } catch (error) {
      setErrorMessage(
            ...
            <>
              <div className="mb-4">
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-4">
                  Import leads from a CSV or JSON file. Make sure your file
                  includes the required fields: name, company, and priority.
                </p>
              </div>
                  ...
                  <div className="bg-gray-100 dark:bg-gray-800 p-4 rounded-lg overflow-hidden">
                    <div className="mb-4">
                      ...
                      <div className="flex gap-3">
                        ...
                        <Button
                          variant={
                            importType === "json" ? "default" : "outline"
                          }
                          ...
                          <div className="mt-3 text-xs text-gray-500">
                            ...
                            <div className="max-w-full">
                              <pre className="bg-gray-200 dark:bg-gray-700 p-2 rounded mt-1 overflow-x-auto whitespace-pre-wrap break-all text-[10px] leading-tight">
                                contact_name,organization,importance,comments,lead_status
                                <br />
                                John Doe,Acme Inc,high,Follow up next week,new
                                <br />
                                Jane Smith,XYZ Corp,medium,Interested in product
                                demo,contacted
                              </pre>
                            </div>
                          </div>
